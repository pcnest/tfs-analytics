...
function buildParams() {
  ...
  const add = (k, v) => { if (v !== null && v !== undefined && String(v).trim() !== "") params.set(k, v); };
...
async function load() {
  qs("tbody").innerHTML = `<tr><td colspan="11" class="muted">Loading…</td></tr>`;
  ...
  if (!data.ok) {
    qs("tbody").innerHTML = `<tr><td colspan="11" class="muted">Error: ${data.error || "unknown"}</td></tr>`;
    return;
  }
  ...
  if (data.rows.length === 0) {
    qs("tbody").innerHTML = `<tr><td colspan="11" class="muted">No rows match the filters.</td></tr>`;
    return;
  }

  qs("tbody").innerHTML = data.rows.map(r => {
    const openDep = (r.openDepCount === null || r.openDepCount === undefined) ? "" : r.openDepCount;
    const openRel = (r.openRelatedCount === null || r.openRelatedCount === undefined) ? "" : r.openRelatedCount;
    return `
      <tr>
        ...
        <td>
          ...
          <div class="muted" style="font-size:12px;">${fmt(r.assignedToUPN)}</div>
        </td>
...
qs("btnLoad").addEventListener("click", () => { offset = 0; load(); });
