...
function buildUpsert(rows) {
  ...
  const valuesSql = rows.map((r, idx) => {
    ...
    values.push(
      ...
      r.openDepCount === null || r.openDepCount === undefined ? null : (normInt(r.openDepCount) ?? 0),

      normInt(r.relatedLinkCount) ?? 0,
      r.openRelatedCount === null || r.openRelatedCount === undefined ? null : (normInt(r.openRelatedCount) ?? 0),
  ...
  const insertSql = `
    INSERT INTO tfs_workitems_lean (${cols.join(",")})
    VALUES ${valuesSql}
...
app.get("/api/lean-workitems", async (req, res) => {
  ...
  if (q) {
    ...
    if (s) {
      ...
      where.push(`(title ILIKE ${p} OR tags ILIKE ${p} OR CAST(work_item_id AS TEXT) ILIKE ${p})`);
    }
  }
...
app.get("/api/lean-workitems/export.csv", async (req, res) => {
  ...
  const {
    q, release, assignedToUPN, state, type, feature, fromChanged, toChanged, limit, offset
  } = req.query;
  ...
  try {
    ...
    const headers = [
      "work_item_id","type","title","state","reason",
      "assigned_to","assigned_to_upn",
      "project","area_path","iteration_path",
      "tags","release",
      "created_by","changed_by",
    ...
    res.setHeader("Content-Disposition", "attachment; filename=tfs_workitems_lean.csv");
...
app.listen(PORT, () => {
  console.log(`tfs-lean-dashboard listening on :${PORT}`);
});
