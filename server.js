...
// AI Insight for Quality & Velocity Metrics
app.get('/api/ai/metrics-insight', async (req, res) => {
  const release = req.query.release ? String(req.query.release).trim() : null;
  const fromDate = req.query.fromDate
    ? String(req.query.fromDate).trim()
    : null;
  ...
  try {
    const quality = await fetchQualityTrendsData({ release, from, to, severity });
    if (!quality.ok) {
      return res.status(500).json({ ok: false, error: quality.error });
    }

    const throughput = await fetchThroughputData({ release, from, to, type });
    if (!throughput.ok) {
      return res.status(500).json({ ok: false, error: throughput.error });
    }

    const ai = await aiService.generateMetricsInsight({
      release,
      qualitySummary: quality.summary,
      qualityWeekly: quality.weekly,
      velocitySummary: throughput.summary,
      velocityWeekly: throughput.weekly,
    ...
    console.error('AI metrics-insight error:', e);
    res.status(500).json({ ok: false, error: String(e?.message || e) });
  }
});

async function fetchQualityTrendsData({ release, from, to, severity }) {
  try {
    const weeklyBugsSql = `
      WITH weekly_created AS (
        SELECT
...
async function fetchThroughputData({ release, from, to, type }) {
  try {
    const sql = `
      WITH weekly_closed AS (
        SELECT
