const CANDIDATE_WORK_ITEMS_SQL = `
  SELECT
    work_item.work_item_id AS "workItemId",
    work_item.type,
    work_item.title,
    work_item.state,
    work_item.release,
    work_item.area_path AS "areaPath",
    work_item.feature_id AS "featureId",
    work_item.feature,
    work_item.weekly_report_remark AS remark,
    work_item.weekly_report_remark_revision AS "remarkRevision",
    work_item.weekly_report_remark_changed_at AS "remarkChangedAt",
    work_item.weekly_report_remark_changed_by AS "remarkChangedBy",
    feature_remark.weekly_report_remark AS "featureRemark",
    feature_remark.weekly_report_remark_revision AS "featureRemarkRevision",
    feature_remark.weekly_report_remark_changed_at AS "featureRemarkChangedAt",
    feature_remark.weekly_report_remark_changed_by AS "featureRemarkChangedBy",
    work_item.tags
  FROM tfs_workitems_analytics work_item
  LEFT JOIN weekly_report_feature_remarks feature_remark
    ON feature_remark.layout_key = $1
    AND feature_remark.feature_work_item_id = work_item.feature_id
  WHERE work_item.is_deleted = FALSE
    AND work_item.area_path = ANY($2::text[])
...
function normalizeWorkItem(row) {
  return {
    ...
    featureRemark: row.featureRemark === null || row.featureRemark === undefined
      ? null
      : String(row.featureRemark),
    featureRemarkRevision: row.featureRemarkRevision === null || row.featureRemarkRevision === undefined || row.featureRemarkRevision === ''
      ? null
      : Number(row.featureRemarkRevision),
    featureRemarkChangedAt: row.featureRemarkChangedAt ?? null,
    featureRemarkChangedBy: row.featureRemarkChangedBy === null || row.featureRemarkChangedBy === undefined
      ? null
      : String(row.featureRemarkChangedBy),
...
async function fetchPlacementInputs(pool, layoutKey, layout) {
  ...
  const [candidateResult, overrideResult] = await Promise.all([
    pool.query(CANDIDATE_WORK_ITEMS_SQL, [layoutKey, layout.source.areaPaths, candidateLimit + 1]),
...
function resolvePlacementPreview({ layoutKey, layout, candidates, overrides = [], candidateOverflow = false, candidateCountLowerBound = null }) {
  ...
  for (const item of items) {
    ...
    placements.push({
      ...
      featureRemark: item.featureRemark,
      featureRemarkRevision: item.featureRemarkRevision,
      featureRemarkChangedAt: item.featureRemarkChangedAt,
      featureRemarkChangedBy: item.featureRemarkChangedBy,
