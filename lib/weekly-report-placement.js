...
function resolvePlacementPreview({ layoutKey, layout, candidates, overrides = [], candidateOverflow = false, candidateCountLowerBound = null }) {
  ...
  for (const item of items) {
    ...
    if (override?.action === 'place') {
      ...
      destination = destinations.get(placementKey) || null;
      ...
      const matches = sortedRules.filter((rule) => matchesPlacementRule(item, rule));
      if (matches.length > 1) {
        warnings.push(validationIssue('warning', 'MULTIPLE_RULE_MATCHES', `Multiple placement rules match; '${matches[0].key}' wins by priority.`, {
          workItemId: item.workItemId,
          path: 'placementRules',
        }));
      }
      if (matches.length > 0) {
        ...
        destination = destinations.get(`${normalizeComparable(matches[0].target.tabKey)}::${normalizeComparable(matches[0].target.sectionKey)}`) || null;
      }
    }
