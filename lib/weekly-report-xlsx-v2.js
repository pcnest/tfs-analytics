...
function buildSectionModel(placements, featureCatalog) {
  ...
  const getReleaseBucket = (release) => {
    const normalizedRelease = release === null || release === undefined || String(release).trim() === ''
      ? null
      : String(release).trim();
      ...
      sections: [...tab.sections]
        ...
        .map((section) => {
          ...
          return {
            ...
            name: section.name,
            order: section.order,
            releases: buildSectionModel(
              placementsByDestination.get(destinationKey) || [],
              featureCatalog,
            ),
          };
        }),
...
function addSectionHeader(worksheet, sectionName) {
  ...
  row.getCell(4).font = { bold: true, color: { argb: 'FF1F4E78' } };
...
async function renderWeeklyReportWorkbook({ layout, preview, generatedAt = new Date(), tfsWorkItemUrlTemplate = '' }) {
  ...
  for (const tab of model.tabs) {
    ...
    for (const section of tab.sections) {
      addSectionHeader(worksheet, section.name);
