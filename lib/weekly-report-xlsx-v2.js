    ...
    if (Number.isInteger(featureId) && featureId > 0) {
      ...
      if (!releaseBucket.featureMap.has(key)) {
        ...
        releaseBucket.featureMap.set(key, {
          ...
          featureRemark: knownFeature?.remark || placement.featureRemark || null,
          items: [],
        });
      }
  ...
  for (const feature of featureRows.values()) {
    if (featureIdsWithChildren.has(feature.workItemId)) {
      for (const bucket of releaseMap.values()) {
        ...
        if (existing) {
          existing.featureTitle = feature.title;
          existing.featureRemark = feature.remark || null;
        }
      }
      continue;
    }
    ...
    releaseBucket.featureMap.set(String(feature.workItemId), {
      ...
      featureRemark: feature.remark || null,
      items: [],
    });
  }
...
function buildFeatureSectionModel(placements, featureCatalog) {
  ...
  const getFeatureBucket = (featureId, featureTitle, featureRemark = null) => {
    const key = String(featureId);
    ...
    if (featureRemark !== null && featureRemark !== undefined) {
      featureMap.get(key).featureRemark = featureRemark;
    }
  ...
  for (const placement of placements) {
    ...
    if (Number.isInteger(featureId) && featureId > 0) {
      ...
      getFeatureBucket(
        ...
        knownFeature?.remark || placement.featureRemark || null,
  ...
  for (const feature of featureRows.values()) {
    ...
    const featureBucket = getFeatureBucket(featureId, feature.title, feature.remark || null);
    featureBucket.featureTitle = feature.title || featureBucket.featureTitle;
    featureBucket.featureRemark = feature.remark || null;
  }
...
function normalizePresentation(presentation) {
  return {
    showWorksheetSummaryBand: presentation?.showWorksheetSummaryBand !== false,
    showFeatureSummary: presentation?.showFeatureSummary !== false,
  };
}
...
function buildWorkbookModel(layout, preview) {
  ...
  return {
    ...
    presentation: normalizePresentation(layout.presentation),
    tabs,
  };
}
...
function addFeatureHeader(worksheet, feature, urlTemplate, showFeatureSummary = true) {
  const row = worksheet.addRow([
    '',
    feature.featureId || '',
    'Feature',
    feature.featureTitle || '',
    showFeatureSummary ? buildFeatureSummary(feature) : '',
    feature.featureRemark || '',
  ]);
  row.eachCell({ includeEmpty: true }, (cell) => {
    cell.fill = {
      type: 'pattern',
...
async function renderWeeklyReportWorkbook({ layout, preview, generatedAt = new Date(), tfsWorkItemUrlTemplate = '' }) {
  ...
  for (const tab of model.tabs) {
    ...
    const frozenRowCount = model.presentation.showWorksheetSummaryBand ? 3 : 2;
    worksheet.views = [{
      state: 'frozen',
      ySplit: frozenRowCount,
      activeCell: `A${frozenRowCount + 1}`,
    }];
    ...
    if (model.presentation.showWorksheetSummaryBand) {
      ...
      if (section.groupingMode === 'feature') {
        for (const feature of section.features) {
          addFeatureHeader(
            ...
            model.presentation.showFeatureSummary,
          );
