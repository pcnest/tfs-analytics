...
function validateLayout(layoutKey, layout) {
  ...
  validateKnownKeys(layout, ['description', 'presentation', 'remarks', 'source', 'filters', 'releaseSelection', 'limits', 'tabs', 'placementRules', 'columns'], root, issues);

  const remarks = layout.remarks;
  if (remarks !== undefined) {
    const remarksPath = `${root}.remarks`;
    if (!isPlainObject(remarks)) {
      issues.push(issue('error', 'INVALID_REMARKS', remarksPath, 'remarks must be an object.'));
    } else {
      validateKnownKeys(remarks, ['source', 'marker', 'clearMarker', 'maxLength'], remarksPath, issues);
      if (remarks.source !== 'tfsDiscussionMarker') {
        issues.push(issue('error', 'INVALID_REMARK_SOURCE', `${remarksPath}.source`, 'source must be "tfsDiscussionMarker".'));
      }
      const markerValid = validateNonEmptyString(remarks.marker, `${remarksPath}.marker`, issues);
      const clearMarkerValid = validateNonEmptyString(remarks.clearMarker, `${remarksPath}.clearMarker`, issues);
      if (
        markerValid &&
        clearMarkerValid &&
        !remarks.clearMarker.toLowerCase().startsWith(remarks.marker.toLowerCase())
      ) {
        issues.push(issue('error', 'INVALID_REMARK_CLEAR_MARKER', `${remarksPath}.clearMarker`, 'clearMarker must begin with marker.'));
      }
      if (!Number.isInteger(remarks.maxLength) || remarks.maxLength < 1 || remarks.maxLength > MAX_REMARK_LENGTH) {
        issues.push(issue('error', 'INVALID_REMARK_LENGTH', `${remarksPath}.maxLength`, `maxLength must be between 1 and ${MAX_REMARK_LENGTH}.`));
      }
    }
  }
  ...
  if (presentation !== undefined) {
    if (!isPlainObject(presentation)) {
      issues.push(issue('error', 'INVALID_PRESENTATION', `${root}.presentation`, 'presentation must be an object.'));
    } else {
      validateKnownKeys(
        presentation,
        ['showWorksheetSummaryBand', 'showFeatureSummary'],
        `${root}.presentation`,
        issues,
      );
      for (const key of ['showWorksheetSummaryBand', 'showFeatureSummary']) {
        if (presentation[key] !== undefined && typeof presentation[key] !== 'boolean') {
          issues.push(issue(
            'error',
            'INVALID_PRESENTATION_SETTING',
            `${root}.presentation.${key}`,
  ...
  const releaseSelection = layout.releaseSelection;
  if (releaseSelection !== undefined) {
    if (!isPlainObject(releaseSelection)) {
      issues.push(issue('error', 'INVALID_RELEASE_SELECTION', `${root}.releaseSelection`, 'releaseSelection must be an object.'));
    } else {
      validateKnownKeys(releaseSelection, ['strategy', 'fallback'], `${root}.releaseSelection`, issues);
      if (!['strict', 'highestNumericTag'].includes(releaseSelection.strategy)) {
        issues.push(issue('error', 'INVALID_RELEASE_SELECTION_STRATEGY', `${root}.releaseSelection.strategy`, 'strategy must be "strict" or "highestNumericTag".'));
      }
      if (releaseSelection.fallback !== 'storedRelease') {
        issues.push(issue('error', 'INVALID_RELEASE_SELECTION_FALLBACK', `${root}.releaseSelection.fallback`, 'fallback must be "storedRelease".'));
      }
    }
  }
    ...
    layout.tabs.forEach((tab, tabIndex) => {
      ...
      tab.sections.forEach((section, sectionIndex) => {
        ...
        if (!isPlainObject(grouping)) {
          ...
          const groupingSignature = JSON.stringify(grouping.levels);
          const supportedGroupingSignatures = new Set([
            JSON.stringify(['release', 'feature']),
            JSON.stringify(['feature']),
          ]);
          if (!supportedGroupingSignatures.has(groupingSignature)) {
            issues.push(issue(
              'error',
              'UNSUPPORTED_GROUPING',
              `${sectionPath}.grouping.levels`,
              'Grouping levels must be ["release", "feature"] or ["feature"].',
            ));
          }
