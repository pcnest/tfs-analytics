...
async function generateReleaseRadarReport(radarData) {
  ...
  const releaseSummaries = radarData
    .map((r) => {
      ...
      const delta = typeof r.confidenceDelta === 'number' ? r.confidenceDelta : 0;
      const deltaLabel =
        delta > 0 ? `+${delta}` : delta < 0 ? `${delta}` : '0';

      return `
Release: ${r.release} (${r.project})
- Confidence: ${r.confidencePct}% (delta ${deltaLabel}, prev ${
        typeof r.previousConfidence === 'number'
...
- In the Confidence column, show the percent AND the delta badge e.g. "100% (<span style='color:#0a7c2f'>+5</span>)"; use green (#0a7c2f) for positive, red (#b42318) for negative, gray (#4b5563) for 0. Always show a sign (+/-/0).
...
- Include the status emoji with the Status value (use the same Go/Watch/No-Go emoji defined above)

For <Insights> per release, provide a concise ~120-150 word analysis that:
...
c) What decision or action is needed (focus on PO/Stakeholder decisions, NOT Engineering/QA tasks)
d) "0% QA Pass Rate" should be explained as "QA has not started" or "QA has no items" if applicable

Use the following guidelines for writing insights:
...
• Write naturally - weave business impact and if needed, decision/support needed into flowing narrative
...
- Display each blocker separated by " | " on separate lines using <br> tags for readability
